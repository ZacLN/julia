...
double conv_to_double(void *data, numerictype_t tag)
{
    ...
    case T_INT64: {
        int64_t l = (int64_t)jl_load_unaligned_i64(data);
        d = (double)l;
        if (d > 0 && l < 0)  // can happen!
            d = -d;
        break;
    }
    case T_UINT64: d = (double)jl_load_unaligned_i64(data); break;
    case T_FLOAT:  d = (double)*(float*)data; break;
    case T_DOUBLE: return jl_load_unaligned_f64(data);
    }
    return d;
}
...
{                                                                                \
    ...
    case T_DOUBLE: i = (type##_t)jl_load_unaligned_f64(data); break;                          \
    }                                                                            \
    return i;                                                                    \
}
...
uint64_t conv_to_uint64(void *data, numerictype_t tag)
{
    ...
    case T_INT64:  i = (uint64_t)jl_load_unaligned_i64(data); break;
    case T_UINT64: i = (uint64_t)jl_load_unaligned_i64(data); break;
    case T_FLOAT:
        if (*(float*)data >= 0)
        ...
        break;
    case T_DOUBLE:
        if (jl_load_unaligned_f64(data) >= 0)
            i = (uint64_t)jl_load_unaligned_f64(data);
        else
            i = (uint64_t)(int64_t)jl_load_unaligned_f64(data);
        break;
    }
    return i;
}
...
int cmp_same_lt(void *a, void *b, numerictype_t tag)
{
    ...
    case T_INT64:  return (int64_t)jl_load_unaligned_i64(a) < (int64_t)jl_load_unaligned_i64(b);
    case T_UINT64: return jl_load_unaligned_i64(a) < jl_load_unaligned_i64(b);
    case T_FLOAT:  return *(float*)a < *(float*)b;
    case T_DOUBLE: return jl_load_unaligned_f64(a) < jl_load_unaligned_f64(b);
    }
    return 0;
}
...
int cmp_same_eq(void *a, void *b, numerictype_t tag)
{
    ...
    case T_INT32:  return *(int32_t*)a == *(int32_t*)b;
    case T_UINT32: return *(uint32_t*)a == *(uint32_t*)b;
    case T_INT64:  return jl_load_unaligned_i64(a) == jl_load_unaligned_i64(b);
    case T_UINT64: return jl_load_unaligned_i64(a) == jl_load_unaligned_i64(b);
    case T_FLOAT:  return *(float*)a == *(float*)b;
    case T_DOUBLE: return jl_load_unaligned_f64(a) == jl_load_unaligned_f64(b);
    }
...
int cmp_lt(void *a, numerictype_t atag, void *b, numerictype_t btag)
{
    ...
    if (atag == T_UINT64) {
        if (btag == T_INT64) {
            if ((int64_t)jl_load_unaligned_i64(b) >= 0) {
                return (jl_load_unaligned_i64(a) < jl_load_unaligned_i64(b));
            }
            return ((int64_t)jl_load_unaligned_i64(a) <
                    (int64_t)jl_load_unaligned_i64(b));
        }
        else if (btag == T_DOUBLE) {
            if (db != db) return 0;
            return (jl_load_unaligned_i64(a) < (uint64_t)jl_load_unaligned_f64(b));
        }
    }
    else if (atag == T_INT64) {
        if (btag == T_UINT64) {
            if ((int64_t)jl_load_unaligned_i64(a) >= 0) {
                return (jl_load_unaligned_i64(a) < jl_load_unaligned_i64(b));
            }
            return ((int64_t)jl_load_unaligned_i64(a) <
                    (int64_t)jl_load_unaligned_i64(b));
            ...
            if (db != db) return 0;
            return ((int64_t)jl_load_unaligned_i64(a) < (int64_t)jl_load_unaligned_f64(b));
        }
    }
    if (btag == T_UINT64) {
        if (atag == T_DOUBLE) {
            if (da != da) return 0;
            return (jl_load_unaligned_i64(b) > (uint64_t)jl_load_unaligned_f64(a));
        }
    }
    else if (btag == T_INT64) {
        if (atag == T_DOUBLE) {
            if (da != da) return 0;
            return ((int64_t)jl_load_unaligned_i64(b) > (int64_t)jl_load_unaligned_f64(a));
        }
    }
    return 0;
}
           ...
           int equalnans)
{
    ...
    if (atag == T_UINT64) {
        ...
        if (btag == T_INT64) {
            ...
            return (jl_load_unaligned_i64(a) == (uint64_t)(int64_t)jl_load_unaligned_f64(b));
        }
    }
    else if (atag == T_INT64) {
        if (btag == T_UINT64) {
        ...
        if (atag == T_DOUBLE) {
            return (jl_load_unaligned_i64(b) == (uint64_t)(int64_t)jl_load_unaligned_f64(a));
        }
    }
    else if (btag == T_INT64) {
